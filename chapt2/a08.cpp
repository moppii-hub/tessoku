...
int main(){
	...
	sum.at(1).at(1) = X.at(1).at(1);
	for(int i=2;i<=H;++i) sum.at(i).at(1) = sum.at(i-1).at(1) + X.at(i).at(1);
	for(int i=2;i<=W;++i) sum.at(1).at(i) = sum.at(1).at(i-1) + X.at(1).at(i);
	for(int i=2;i<=H;++i) for(int j=2;j<=W;++j) sum.at(i).at(j) = X.at(i).at(j) + sum.at(i-1).at(j) + sum.at(i).at(j-1) - sum.at(i-1).at(j-1);
